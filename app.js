...
// Rota para retornar o gráfico de barras com dados dos professores
app.get('/reservas/professores', (req, res) => {
    try {
        // Consulta para buscar todas as reservas
        db.ref('confirmacao').once('value', (snapshot) => {
            const reservas = snapshot.val();
            const reservasPorProfessor = {};

            // Agrupando as reservas por professor
            for (const key in reservas) {
                const reserva = reservas[key];
                const professor = reserva.nome; // Aqui assumimos que "nome" é o nome do professor

                if (!reservasPorProfessor[professor]) {
                    reservasPorProfessor[professor] = 0;
                }
                reservasPorProfessor[professor]++;
            }

            // Convertendo o objeto de reservas por professor em um array para enviar ao frontend
            const result = Object.keys(reservasPorProfessor).map((professor) => {
                return {
                    professor: professor,
                    quantidade: reservasPorProfessor[professor]
                };
            });
        ...
        console.log(e);
        res.status(500).json({ error: 'Erro ao buscar reservas por professores' });
    }
});

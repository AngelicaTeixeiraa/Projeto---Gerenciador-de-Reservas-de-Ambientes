...
function criarTabela(dados) {
    let tabela = `<table class="table table-striped hover" id="tabela-analise">
                    <thead>
                        <tr>
    ...
    for (let chave in dados) {
        if (dados[chave].status == "Aguardando")
            var botaoAnalisar =  `<a class="btn btn-outline-warning" href="/confirmacao/analisar/${chave}">Analisar</a>`;
        else if (dados[chave].status)
            var botaoAnalisar =  `Aceito`;
        else
            var botaoAnalisar =  `Recusado`;
...
function criarTabelaUsu(dados) {
    ...
    for (let chave in dados) {
        ...
        const dia = dados[chave].data.substring(8, 10);
        if (dados[chave].status == "Aguardando")
            var botaoAnalisar =  `Aguardando análise`;
        else if (dados[chave].status)
            var botaoAnalisar =  `Aceito`;
        else
            var botaoAnalisar =  `Recusado`;
...
// Aceitar solicitação
app.get('/aceitar/:id', urlencodedParser, (req, res) => {
    try {
        let id = req.params.id; // Pega o status da URL (aceitar ou recusar)
        ...
        docConfirmacao.child(id).update(
            {
                'status': true // Condicional: aceita se 'aceitar', caso contrário recusa
            }
        );
...
// Aceitar solicitação
app.get('/recusar/:id', urlencodedParser, (req, res) => {
    try {
        let id = req.params.id; // Pega o status da URL (aceitar ou recusar)
        ...
        docConfirmacao.child(id).update(
            {
                'status': false // Condicional: aceita se 'aceitar', caso contrário recusa
            }
        );
        res.redirect("/confirmacao");
    } catch (e) {
